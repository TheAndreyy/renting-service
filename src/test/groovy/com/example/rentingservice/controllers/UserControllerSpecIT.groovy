...
@Transactional
class UserControllerSpecIT extends WebMvcSpecIT {
    ...
    def "It should properly return reservation list for user"() {
        ...
        def object = objectRepository.findById(3).get()
        ...
        when:
        def result = mvc.perform(get(ApplicationConstants.API_PREFIX + "/user/$userId/reservations")
    ...
    def "It should return bad request for non existing user"() {
        given:
        def userId = "300"
        ...
        result.andExpect(status().isNotFound())
                .andExpect(jsonPath("\$.message").value("User with id: 300 not found"))
    }
